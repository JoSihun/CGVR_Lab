...
//    }
    ...
    public String laboratoryNoticePost(@PathVariable Long id, Model model, HttpSession session) {
    ...
    /** 파일 다운로드 */
    @GetMapping(value = "/laboratory/notice/posts/{postsId}/download/{attachId}", produces = MediaType.APPLICATION_OCTET_STREAM_VALUE)
    ...
    public String laboratoryNoticePostUpdate(@PathVariable Long id, Model model, HttpSession session) {
        PostsResponseDto responseDto = this.postsService.findById(id);
